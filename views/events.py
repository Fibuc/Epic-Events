...
class EventView:
    ...
    @staticmethod
    def select_event_to_modify():
        return console.input("Quel est l'ID de l'événement à modifier? : ")

    @staticmethod
    def get_date_time(end_date=False, modify=False):
        date = console.input(
            f"Quelle est la {'nouvelle ' if modify else ''}date de "
            f"{'fin' if end_date else 'début'}? (JJ/MM/AAAA) : "
        )
        time = console.input(
            f"Quelle est la {'nouvelle ' if modify else ''}heure de "
            f"{'fin' if end_date else 'début'}? (HH:MM): "
        )
        return date, time
    ...
    def show_all_events(all_events):
        table = get_table(
            'Evénements', [
                'ID', 'Client', 'N°Contrat', 'Début Evénement',
                'Fin Evénement', 'Support', 'Lieu', 'Nb. Participants', 'Notes'
            ]
        )
        for event in all_events:
            end_date = (
                '[italic black]Aucune[/italic black]'
                if not event.event_end
                else convert_date(event.event_end))
            support_name = (
                '[italic black]Aucun[/italic black]'
                if not event.support
                else event.support.full_name)
            notes = (
                '[italic black]Aucunes[/italic black]'
                if not event.notes
                else event.notes
            )
            table.add_row(
    ...
    def get_information_to_modify(event):
        ...
        table.add_row('1', 'Contrat', contract)
    ...
    def creation_success_message():
        console.print(
            "L'événement a été créé avec succès.", style='success'
        )
    ...
    def get_event_contract(all_contracts):
        ...
        table = get_table(
            'Contrats',
            ['ID', 'Client', 'Commercial', 'Montant total', 'Statut']
        )
    ...
    def invalid_datetime(date_time):
        console.print(
            f"La date \"{date_time}\" ne respecte pas le format "
            f"\"JJ/MM/AAAA hh:mm\".", style='error'
        )

    @staticmethod
    def invalid_attendees(attendees):
        console.print(
            f"Le nombre de participants \"{attendees}\" n'est pas valide.",
            style='error'
        )
    ...
    def get_location(modify=False):
        return console.input(
            f"Quelle est l{"a nouvelle " if modify else "'"}adresse de "
            f"l'événement? : "
        )
    ...
    def get_attendees(modify=False):
        return console.input(
            f"Quel est le {"nouveau " if modify else ''}nombre de "
            f"participants? : "
        )
    ...
    def get_notes(modify=False):
        return console.input(
            f"{'' if modify else '[italic][Facultatif][/italic] '}Ajouter "
            f"des notes? : "
        )
    ...
    def unauthorized_modification():
        console.print(
            "Accès refusé: Vous n'êtes pas autorisé à modifier cet événement.",
            style='error'
        )
    ...
    def not_support(support_id):
        console.print(
            f"Erreur: L'utilisateur ayant pour ID {support_id} n'existe pas "
            f"ou n'est pas support.", style='error'
        )
    ...
    def confirm_support_choice(support_name):
        return console.input(
            f"Saisissez '[bold]Oui[/bold]' pour confirmer l'association de "
            f"{support_name} à l'événement : "
        )

    @staticmethod
    def cancel_modifications():
        console.print(
            "La modification n'a pas été effectuée.", style='italic'
        )

    @staticmethod
    def error_past_date():
        console.print(
            "Erreur: La date désirée est antérieure à la date du début "
            "l'événement", style='error'
        )

    @staticmethod
    def null_end_date():
        console.print(
            "Avertissement: La date de fin de l'événement a été supprimée "
            "car elle est antérieure à la date de début.", style='warning'
        )
    ...
    def get_support(all_supports):
        ...
        console.print(table)
        return console.input(
            "Quel est l'ID du support à associer au contrat? : "
        )
