...
class ContractView:
    ...
    @staticmethod
    def select_contract(option):
        return console.input(f"Quel est l'ID du contrat à {option}? : ")

    @staticmethod
    def get_information_to_modify(contract):
        commercial_name = ['-' if contract.commercial == None else contract.commercial.full_name][0]
        space()
    ...
    def show_all_contracts(all_contracts):
        space()
        table = get_table('Contrats', ['ID', 'Client', 'Commercial', 'Montant total', 'Déjà réglé', 'Restant dû', 'Statut', 'Date création', 'Date modification'])
        for contract in all_contracts:
            sold = [str(contract.outstanding_balance) if contract.outstanding_balance > 0 else '[italic bold bright_green]Soldé[/italic bold bright_green]'][0]
    ...
    def not_digit(number):
        console.print(f"Votre choix '{number}' n'est pas un nombre.", style='error')
