...
COLORS = [
    'green', 'cyan', 'blue', 'magenta', 'dark_green',
    'bright_blue', 'yellow', 'white', 'white'
]
...
def get_success_error_console():
    theme = Theme(
        {'success': 'green', 'error': 'bold red', 'warning': 'bold yellow'}
    )
...
def get_table(title: str, column_names: list[str]):
    ...
    for i, name in enumerate(column_names):
        if name in [
            'Numéro', 'ID', 'Montant total', 'Déjà réglé',
            'N°Contrat', 'Restant dû'
        ]:
